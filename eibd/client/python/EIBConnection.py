...
#                 2006- prz@zeta2.ch, fixed up local sockets not resetting
#
...
class EIBConnection(object):
  ...
  def __EIB_CheckRequest(self, block):
    ...
    if not self.remote and self.readlen ==0:
      self.errno = errno.ECONNRESET
      return -1
    elif self.readlen < 2:
      return 0
    ...
    if not self.remote and self.readlen ==0:
      self.errno = errno.ECONNRESET
      return -1
    elif self.readlen < self.datalen + 2:
      self.fd.setblocking(block)
  ...
  def EIBOpenT_Group_async(self, dest, write_only, priority):
    ibuf = [0] * 6;
    ibuf[2] = ((dest>>8)&0xff)
    ibuf[3] = ((dest)&0xff)
    if write_only != 0:
  ...
  def EIBOpenT_Group(self, dest, write_only, priority):
    if self.EIBOpenT_Group_async (dest, write_only, priority) == -1:
      return -1
    return self.EIBComplete ()
  ...
  def __EIB_State_Threads_Complete(self):
    self.__complete = None;
    if self.__EIB_GetRequest() == -1:
      return -1;
    if (((self.data[0])<<8)|(self.data[0+1])) != 257 or len(self.data) < 2:
      self.errno = errno.ECONNRESET
      return -1
    self.buf.buffer = self.data[2:]
    return len(self.buf.buffer)
  ...
  def EIB_State_Threads_async(self, buf):
    ibuf = [0] * 2;
    self.buf = buf
    ibuf[0] = 1
    ibuf[1] = 1
    if self.__EIB_SendRequest(ibuf) == -1:
      return -1;
    self.__complete = self.__EIB_State_Threads_Complete;
    return 0


  def EIB_State_Threads(self, buf):
    if self.EIB_State_Threads_async (buf) == -1:
      return -1
    return self.EIBComplete ()

  def __EIB_State_Backends_Complete(self):
    self.__complete = None;
    if self.__EIB_GetRequest() == -1:
      return -1;
    self.buf.buffer = self.data[2:]
  ...
  def EIB_State_Backends_async(self, buf):
    ibuf = [0] * 2;
    self.buf = buf
    ibuf[0] = 1
    ibuf[1] = 2
    if self.__EIB_SendRequest(ibuf) == -1:
      return -1;
    self.__complete = self.__EIB_State_Backends_Complete;
    return 0


  def EIB_State_Backends(self, buf):
    if self.EIB_State_Backends_async (buf) == -1:
      return -1
    return self.EIBComplete ()

  def __EIB_State_Servers_Complete(self):
    self.__complete = None;
    if self.__EIB_GetRequest() == -1:
      return -1;
    self.buf.buffer = self.data[2:]
  ...
  def EIB_State_Servers_async(self, buf):
    ibuf = [0] * 2;
    self.buf = buf
    ibuf[0] = 1
    ibuf[1] = 3
    if self.__EIB_SendRequest(ibuf) == -1:
      return -1;
    self.__complete = self.__EIB_State_Servers_Complete;
    return 0


  def EIB_State_Servers(self, buf):
    if self.EIB_State_Servers_async (buf) == -1:
      return -1
    return self.EIBComplete ()
...
IMG_KEY_WRITE = 61
XMLEIBDNAMESPACE = "http://www.zeta2.ch/xml/eibd"
XMLSTATUSELEMENT = "status"
XMLSTATUSVERSIONATTR = "version"
XMLSTATUSSTARTTIMEATTR = "start-time"
XMLSTATDROPSATTR = "drops"
XMLSTATSENDERRATTR = "send-errors"
XMLSTATRECVERRATTR = "receive-errors"
XMLSTATRESETSATTR = "resets"
XMLSTATUSUP = "up"
XMLSTATUSDOWN = "down"
XMLSTATUSUNKNOWN = "unknown"
XMLBACKENDELEMENT = "backend"
XMLBACKENDELEMENTTYPEATTR = "type"
XMLBACKENDSTATUSATTR = "status"
XMLBACKENDADDRESSATTR = "address"
XMLDRIVERELEMENT = "driver"
XMLDRIVERELEMENTATTR = "type"
XMLDRIVERSTATUSATTR = "status"
XMLDRIVERDEVICEATTR = "device"
XMLQUEUESTATELEMENT = "queue"
XMLQUEUENAMEATTR = "name"
XMLQUEUEMAXLENATTR = "maximum-length"
XMLQUEUEMAXREACHLENATTR = "maximum-reached-length"
XMLQUEUECURRENTLENATTR = "current-length"
XMLQUEUEINSERTSATTR = "inserts"
XMLQUEUEDROPSATTR = "drops"
XMLQUEUEMAXDELAYATTR = "maximum-delay"
XMLQUEUEMEANDELAYATTR = "mean-delay"
XMLSERVERELEMENT = "server"
XMLSERVERTYPEATTR = "type"
XMLSERVERADDRESSATTR = "listen-address"
XMLSERVERMAXCLIENTSATTR = "max-reached-clients"
XMLSERVERMAXALLOWCLIENTSATTR = "max-allowed-clients"
XMLSERVERCLIENTSATTR = "current-clients"
XMLSERVERCLIENTSTOTALATTR = "total-clients"
XMLSERVERCLIENTSREJECTEDATTR = "clients-rejected"
XMLSERVERCLIENTSAUTHFAILATTR = "clients-authentication-failed"
XMLSERVERMESSAGESRECVATTR = "packets-received-all-clients"
XMLSERVERMESSAGESSENTATTR = "packets-sent-all-clients"
XMLSERVERSENDERRATTR = "send-errors-all-clients"
XMLSERVERRECVERRATTR = "receive-errors-all-clients"
XMLSERVERXMLPROTODEFATTR = "client-protocol-namespace"
XMLSERVERSUBNETFILTERS = "subnet-filters"
XMLCLIENTELEMENT = "client"
XMLCLIENTTYPEATTR = "type"
XMLCLIENTSTARTTIMEATTR = "start-time"
XMLCLIENTSMESSAGESRECVATTR = "packets-received"
XMLCLIENTSMESSAGESSENTATTR = "packets-sent"
XMLCLIENTSRECVERRATTR = "receive-errors"
XMLCLIENTSSENDERRATTR = "send-errors"
XMLCLIENTADDRESSATTR = "address"
XMLCLIENTSTATEADDR = "state"
EIBD_LOG_EMERG = "emerg"
EIBD_LOG_ALERT = "alert"
EIBD_LOG_CRIT = "crit"
EIBD_LOG_ERR = "error"
EIBD_LOG_WARN = "warn"
EIBD_LOG_NOTICE = "note"
EIBD_LOG_INFO = "info"
EIBD_LOG_DEBUG = "debug"
PRIO_LOW = 3
PRIO_NORMAL = 1
PRIO_URGENT = 2
PRIO_SYSTEM = 0
