...
class EIBConnection
{
  ...
  private function EIB_Cache_LastUpdates_complete ()
  {
    if ($this->_EIB_GetRequest () == -1)
      return -1;
    if (((EIBConnection::upack (substr ($this->data, 0, 2), "n"))) != 0x0076
	|| strlen ($this->data) < 2)
      {
	$this->errno = self::ECONNRESET;
	return -1;
  ...
  public function EIB_Cache_LastUpdates_async ($start, $timeout,
					       EIBBuffer $buf, EIBInt16 $ende)
  {
    $ibuf = array ();
    for ($i = 0; $i < 5; $i++)
      $ibuf[$i] = " ";
  ...
  public function EIB_Cache_LastUpdates ($start, $timeout, EIBBuffer $buf,
					 EIBInt16 $ende)
  {
    if ($this->EIB_Cache_LastUpdates_async ($start, $timeout, $buf, $ende) ==
	-1)
      return -1;
    return $this->EIBComplete ();
  }
  ...
  private function EIB_MC_Individual_Open_complete ()
  {
    if ($this->_EIB_GetRequest () == -1)
      return -1;
    if (((EIBConnection::upack (substr ($this->data, 0, 2), "n"))) != 0x0049
	|| strlen ($this->data) < 2)
      {
	$this->errno = self::ECONNRESET;
	return -1;
  ...
  public function EIB_MC_PropertyDesc ($obj, $propertyno, EIBInt8 $proptype,
				       EIBInt16 $max_nr_of_elem,
				       EIBInt8 $access)
  {
    if ($this->
	EIB_MC_PropertyDesc_async ($obj, $propertyno, $proptype,
				   $max_nr_of_elem, $access) == -1)
      return -1;
    return $this->EIBComplete ();
  }
  ...
  public function EIB_MC_PropertyRead_async ($obj, $propertyno, $start,
  ...
  public function EIB_MC_PropertyRead ($obj, $propertyno, $start, $nr_of_elem,
				       EIBBuffer $buf)
  {
    if ($this->
	EIB_MC_PropertyRead_async ($obj, $propertyno, $start, $nr_of_elem,
				   $buf) == -1)
      return -1;
    return $this->EIBComplete ();
  }
  ...
  public function EIB_MC_PropertyWrite_async ($obj, $propertyno, $start,
					      $nr_of_elem, $buf,
					      EIBBuffer $res)
  {
    $ibuf = array ();
  ...
  public function EIB_MC_PropertyWrite ($obj, $propertyno, $start,
					$nr_of_elem, $buf, EIBBuffer $res)
  {
    if ($this->
	EIB_MC_PropertyWrite_async ($obj, $propertyno, $start, $nr_of_elem,
				    $buf, $res) == -1)
      return -1;
    return $this->EIBComplete ();
  }
					...
					AGARG_BOOLa (priority, ARG_NONE))
  {
    ...
    $ibuf[2 + 1] = EIBConnection::packb (($dest) & 0xff);
    $ibuf[4] = EIBConnection::packb (($write_only) ? 0xff : 0);
    $ibuf[5] = EIBConnection::packb (($priority) & 0xff);
  ...
  {
    if ($this->
	EIBOpenT_Group_async ($dest, $write_only,
			      ALARG_BOOLa (priority, ARG_NONE)) == -1)
      return -1;
    return $this->EIBComplete ();
  }
  ...
  private function EIB_State_Threads_complete ()
  {
    if ($this->_EIB_GetRequest () == -1)
      return -1;
    if (((EIBConnection::upack (substr ($this->data, 0, 2), "n"))) != 0x0101
	|| strlen ($this->data) < 2)
      {
	$this->errno = self::ECONNRESET;
	return -1;
  ...
  public function EIB_State_Threads_async (EIBBuffer $buf)
  {
    $ibuf = array ();
    for ($i = 0; $i < 2; $i++)
      $ibuf[$i] = " ";
    $this->buf = $buf;
    $ibuf[0] = EIBConnection::packb ((0x0101 >> 8) & 0xff);
    $ibuf[1] = EIBConnection::packb ((0x0101) & 0xff);
    if ($this->_EIB_SendRequest (implode ("", $ibuf)) == -1)
      return -1;
    $this->complete = "EIB_State_Threads"."_complete";
    return 0;
  }
  public function EIB_State_Threads (EIBBuffer $buf)
  {
    if ($this->EIB_State_Threads_async ($buf) == -1)
      return -1;
    return $this->EIBComplete ();
  }
  private function EIB_State_Backends_complete ()
  {
    if ($this->_EIB_GetRequest () == -1)
      return -1;
    $this->buf->buffer = substr ($this->data, 2);
  ...
  public function EIB_State_Backends_async (EIBBuffer $buf)
  {
    $ibuf = array ();
    for ($i = 0; $i < 2; $i++)
      $ibuf[$i] = " ";
    $this->buf = $buf;
    $ibuf[0] = EIBConnection::packb ((0x0102 >> 8) & 0xff);
    $ibuf[1] = EIBConnection::packb ((0x0102) & 0xff);
    if ($this->_EIB_SendRequest (implode ("", $ibuf)) == -1)
      return -1;
    $this->complete = "EIB_State_Backends"."_complete";
    return 0;
  }
  public function EIB_State_Backends (EIBBuffer $buf)
  {
    if ($this->EIB_State_Backends_async ($buf) == -1)
      return -1;
    return $this->EIBComplete ();
  }
  private function EIB_State_Servers_complete ()
  {
    if ($this->_EIB_GetRequest () == -1)
      return -1;
    $this->buf->buffer = substr ($this->data, 2);
  ...
  public function EIB_State_Servers_async (EIBBuffer $buf)
  {
    $ibuf = array ();
    for ($i = 0; $i < 2; $i++)
      $ibuf[$i] = " ";
    $this->buf = $buf;
    $ibuf[0] = EIBConnection::packb ((0x0103 >> 8) & 0xff);
    $ibuf[1] = EIBConnection::packb ((0x0103) & 0xff);
    if ($this->_EIB_SendRequest (implode ("", $ibuf)) == -1)
      return -1;
    $this->complete = "EIB_State_Servers"."_complete";
    return 0;
  }
  public function EIB_State_Servers (EIBBuffer $buf)
  {
    if ($this->EIB_State_Servers_async ($buf) == -1)
      return -1;
    return $this->EIBComplete ();
  }
