...
sub EIBOpenT_Group_async {
    use bytes;
    my($self, $dest, $write_only, $priority) = @_;
    my $i;
    my(@ibuf) = {};
    for ($i = 0; $i < 6; ++$i) {
        $ibuf[$i] = 0;
...
sub EIBOpenT_Group {
    use bytes;
    my($self, $dest, $write_only, $priority) = @_;
    return undef unless $self->EIBOpenT_Group_async($dest, $write_only, $priority) == 0;
    return $self->EIBComplete;
}
...
sub EIB_State_Threads_complete {
    use bytes;
    my $self = shift();
    $$self{'complete'} = undef;
    my $i;
    return undef unless $self->_EIB_Get_Request == 0;
    if ((vec($$self{'data'}, 0, 8) << 8 | vec($$self{'data'}, 1, 8)) != 257 or bytes::length($$self{'data'}) < 2) {
        $! = &Errno::ECONNRESET;
        return undef;
    }
    ${$$self{'buf'};} = substr($$self{'data'}, 2);
...
sub EIB_State_Threads_async {
    ...
    $ibuf[1] = 1;
    return undef unless $self->_EIB_SendRequest(pack('C' . @ibuf, @ibuf)) == 0;
    $$self{'complete'} = 'EIB_State_Threads_complete';
    return 0;
}
sub EIB_State_Threads {
    use bytes;
    my($self, $buf) = @_;
    return undef unless $self->EIB_State_Threads_async($buf) == 0;
    return $self->EIBComplete;
}
sub EIB_State_Backends_complete {
    use bytes;
    my $self = shift();
    $$self{'complete'} = undef;
    my $i;
...
sub EIB_State_Backends_async {
    ...
    $ibuf[1] = 2;
    return undef unless $self->_EIB_SendRequest(pack('C' . @ibuf, @ibuf)) == 0;
    $$self{'complete'} = 'EIB_State_Backends_complete';
    return 0;
}
sub EIB_State_Backends {
    use bytes;
    my($self, $buf) = @_;
    return undef unless $self->EIB_State_Backends_async($buf) == 0;
    return $self->EIBComplete;
}
sub EIB_State_Servers_complete {
    use bytes;
    my $self = shift();
    $$self{'complete'} = undef;
    my $i;
...
sub EIB_State_Servers_async {
    ...
    $ibuf[1] = 3;
    return undef unless $self->_EIB_SendRequest(pack('C' . @ibuf, @ibuf)) == 0;
    $$self{'complete'} = 'EIB_State_Servers_complete';
    return 0;
}
sub EIB_State_Servers {
    use bytes;
    my($self, $buf) = @_;
    return undef unless $self->EIB_State_Servers_async($buf) == 0;
    return $self->EIBComplete;
}
sub EIBSocketLocal {
    use bytes;
