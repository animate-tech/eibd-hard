...
/** Opens a connection of type T_Group setting write priority.
 * \param con eibd connection
 * \param dest group address
 * \param write_only if not null, no packets from the bus will be delivered
 * \return 0 if successful, -1 if error
 */
int EIBOpenT_Group (EIBConnection * con, eibaddr_t dest, int write_only, int priority);
 ...
 * \return 0 if started, -1 if error
 */
int EIBOpenT_Group_async (EIBConnection * con, eibaddr_t dest, int write_only, int priority);
...
/** Dump the status of the threads in EIBD.
 * \param con eibd connection
 * \param max_len buffer size
 * \param buf buffer for state output. ASCII text.
 * \return 0 if started, -1 if error
 */
 int EIB_State_Threads (EIBConnection * con,
        int maxlen, uint8_t * buf);

/** Dump the status of EIBD backend.
 * \param con eibd connection
 * \param max_len buffer size
 * \param buf buffer for state output. ASCII text.
 * \return 0 if started, -1 if error
 */
int EIB_State_Backends (EIBConnection * con,
        int maxlen, uint8_t * buf);

/** Dump the status of EIBD servers.
 * \param con eibd connection
 * \param max_len buffer size
 * \param buf buffer for state output. ASCII text.
 * \return 0 if started, -1 if error
 */
int EIB_State_Servers (EIBConnection * con,
        int maxlen, uint8_t * buf);
