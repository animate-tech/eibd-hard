	    ...
	    const struct sockaddr_in *src, bool & nat)
{
  int ip, port;
  memset (a, 0, sizeof (*a));
  if (buf[0] != 0x8 || buf[1] != 0x1) // only UDP accepted
    return 1;
...
const char EIBNetIPSocket::indropmsg[] = "EIBNetIPSocket: incoming queue length exceeded, dropping packet";
const char EIBNetIPSocket::outdropmsg[] = "EIBNetIPSocket: outgoing queue length exceeded, dropping packet";

Element * EIBNetIPSocket::_xml(Element *parent) const
{
  Element *p = parent->addElement(XMLDRIVERELEMENT);
  p->addAttribute(XMLDRIVERELEMENTATTR, _str());
  p->addAttribute(XMLDRIVERSTATUSATTR, XMLSTATUSUP);
  inqueue._xml(p);
  outqueue._xml(p);
...
EIBNetIPSocket::EIBNetIPSocket (struct sockaddr_in bindaddr, bool reuseaddr,
  ...
  Thread(tr,PTH_PRIO_STD, "EIBNetIPSocket"),
  outqueue("EIBNet/IP output", outquemaxlen),
  inqueue("EIBNet/IP input", inquemaxlen),
  ipnetfilters(ipnetfilters)
{
  int i;
  TRACEPRINTF (Thread::Loggers(), 0, this, "Open");
  multicast = 0;
  ...
  if (fd == -1)
    {
      ERRORLOGSHAPE (tr, LOG_ERR, Logging::DUPLICATESMAX1PER10SEC, this, Logging::MSGNOHASH,
                "Device Socket Open for EIBNet/IP Failed (can't open datagram socket)");
  ...
  if (reuseaddr)
    {
      ...
      if (setsockopt (fd, SOL_SOCKET, SO_REUSEADDR, &i, sizeof (i)) == -1)
        {
          ERRORLOGSHAPE (tr, LOG_ERR, Logging::DUPLICATESMAX1PER10SEC, this, Logging::MSGNOHASH,
                "Device Socket Configuration for EIBNet/IP Failed (can't set reuse)");
	      throw Exception (DEV_OPEN_FAIL);
        }
    }
  if (bind (fd, (struct sockaddr *) &bindaddr, sizeof (bindaddr)) == -1)
    {
      ERRORLOGSHAPE (tr, LOG_ERR, Logging::DUPLICATESMAX1PER10SEC, this, Logging::MSGNOHASH,
                "Device Socket Bind for EIBNet/IP Failed (can't bind)");
      throw Exception (DEV_OPEN_FAIL);
    }

  Start ();
  TRACEPRINTF (Thread::Loggers(), 0, this, "Socket Opened");
}

EIBNetIPSocket::~EIBNetIPSocket ()
{
  TRACEPRINTF (Thread::Loggers(), 0, this, "Close");
  Stop ();
  if (fd != -1)
    {
      if (multicast)
                              ...
                              Logs * tr)
{
  if (multicast)
    {
      ERRORLOGSHAPE (tr, LOG_ERR, Logging::DUPLICATESMAX1PER10SEC, this, Logging::MSGNOHASH,
                "Device Socket Configuration for EIBNet/IP Failed (can't set multicast)");
      throw Exception (DEV_OPEN_FAIL);
    }
  maddr = multicastaddr;
  if (setsockopt (fd, SOL_IP, IP_ADD_MEMBERSHIP, &maddr, sizeof (maddr)) ==
    ...
    {
      ERRORLOGSHAPE (tr, LOG_ERR, Logging::DUPLICATESMAX1PER10SEC, this, Logging::MSGNOHASH,
                "Device Socket Configuration for EIBNet/IP Failed (can't set multicast membership)");
      return false;
    }
  multicast = 1;
  return true;
...
{
   _EIBNetIP_Send s;
  Thread::Loggers()->TracePacket (1, this, "Send", p.data);
  s.data = p;
  s.addr = sendaddr;

  return Put_On_Queue_Or_Drop< _EIBNetIP_Send,  _EIBNetIP_Send>(inqueue, s, &insignal, true, indropmsg);
#if 0
  inqueue.put (s);
  ...
  if (s)
    {
      ...
      Thread::Loggers()->TracePacket (1, this, "Recv", outqueue.top ().data);
...
EIBNetIPSocket::Run (pth_sem_t * stop1)
{
  ...
  while (pth_event_status (stop) != PTH_STATUS_OCCURRED)
	...
	pth_recvfrom_ev (fd, buf, sizeof (buf), 0, (struct sockaddr *) &r,
      ...
      if (i > 0 && rl == sizeof (r))
        {
          bool infilters = ipnetfilters.size()==0;
          if (ipnetfilters.size() > 0)
            {
              for (IPv4NetList::const_iterator ii = ipnetfilters.begin();
                  ii != ipnetfilters.end() && !infilters; ii++)
                {
                  infilters |= ii->contains(IPv4(r));
                }
              if (!infilters)
                {
                  WARNLOGSHAPE(Thread::Loggers(), LOG_WARNING,
                      Logging::DUPLICATESMAX1PER10SEC, this, Logging::MSGNOHASH,
                      "Tunneled/Routed packet from addr %s rejected due to subnet filtering", get_addr_str((struct sockaddr *) &r));
                }
            }
          if (infilters)
            {
              if (recvall == 1 || !memcmp(&r, &recvaddr, sizeof(r))
                  ...
                  || (recvall == 3 && !memcmp(&r, &recvaddr2, sizeof(r))))
                {
                  ...
                  EIBNetIPPacket *p = EIBNetIPPacket::fromPacket(CArray(buf, i),
                      r);
                  if (p)
                    {
                      Put_On_Queue_Or_Drop<EIBNetIPPacket, EIBNetIPPacket>(
                          outqueue, *p, &outsignal, true, outdropmsg);
#if 0
                      outqueue.put (*p);
                      delete p;
      ...
      if (!inqueue.isempty ())
	{
	  const _EIBNetIP_Send s = inqueue.top ();
	  CArray p = s.data.ToPacket ();
	  Thread::Loggers()->TracePacket (0, this, "Send", p);
	  i =
	  ...
	  if (error > 5)
	    {
	      Thread::Loggers()->TracePacket (0, this, "Drop EIBnetSocket", p);
	      pth_sem_dec (&insignal);
	      inqueue.get ();
	      error = 0;
	    }
...
template bool DroppableQueueInterface::Put_On_Queue_Or_Drop<EIBNetIPPacket, EIBNetIPPacket>
(Queue<EIBNetIPPacket>&, EIBNetIPPacket, pth_sem_st*, bool, char const*, pth_sem_st*, int);
