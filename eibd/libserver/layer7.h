...
class Layer7_Broadcast : public DroppableQueueInterface, public LoggableObjectInterface

{
  T_Broadcast *l4;

public:
    Layer7_Broadcast (Layer3 * l3, Logs * tr,  int inquemaxlen =0, int outquemaxlen =0, int peerquemaxlen =0);
  ...
  const char *str_(void) { return "[L7]"; };
};

/** Layer 7 Individual Connection */
class Layer7_Connection : public DroppableQueueInterface, public LoggableObjectInterface
{
  T_Connection *l4;
  /** destination address */
  eibaddr_t dest;
...
public:
  Layer7_Connection (Layer3 * l3, eibaddr_t dest,
		     Logs * tr,
		     int inquemaxlen =QUEUE_UNLIMITED_LEN,
		     int outquemaxlen =QUEUE_UNLIMITED_LEN,
		     int peerquemaxlen =QUEUE_UNLIMITED_LEN);
...
/** Layer 7 Individual  */
class Layer7_Individual : public DroppableQueueInterface, public LoggableObjectInterface
{
  T_Individual *l4;
  /** destination address */
  eibaddr_t dest;
  ...
  APDU *Request_Response (APDU * r);
public:
    Layer7_Individual (Layer3 * l3, Logs * tr, eibaddr_t dest,  int inquemaxlen, int outquemaxlen, int peerquemaxlen);
