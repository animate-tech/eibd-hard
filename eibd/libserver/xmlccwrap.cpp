...
void errorFunc(void *ctx, const char *msg, ...)
{
  ...
  va_list args;
  va_start(args, msg);
  vsnprintf(buffer, MAX_BUF, msg, args);
  va_end(args);
...
{
  xsltP = xsltParseStylesheetFile((xmlChar *)fn.c_str());
  if (xsltP)
    return true;
  return false;
}
...
Element *XMLTree::GenerateEIBDStatusRoot(void)
{
  setEncoding("UTF-8");

  Element * rtixml = new Element("eibd");
  _root = rtixml;
  rtixml->addAttribute("xmlns",XMLEIBDNAMESPACE);
  rtixml->addAttribute("xmlns:xsi","http://www.w3.org/2001/XMLSchema-instance");
  rtixml->addAttribute("xsi:schemaLocation",XMLEIBDNAMESPACE "/" VERSION " "
		       XMLEIBDNAMESPACE "/" VERSION "/eibd.xsd");

  rtixml->addAttribute("xmlns:xlink","http://www.w3.org/1999/xlink");
  return _root;
}

Element *XMLTree::GenerateGraphMLRoot(void)
{
  setEncoding("UTF-8");
  ...
  graphml->addAttribute("xmlns:xsi","http://www.w3.org/2001/XMLSchema-instance");
  graphml->addAttribute("xsi:schemaLocation","http://graphml.graphdrawing.org/xmlns/graphml "
			"http://www.yworks.com/xml/schema/graphml/1.0/ygraphml.xsd");
  graphml->addAttribute("xmlns:y","http://www.yworks.com/xml/graphml");
...
const CArray & XMLTree::writeCArray() const
{
  static CArray retval;
  unsigned char *ptr;
  int len;
  xmlDocPtr doc;
  ElementList children;
...
Element *Element::addContent(const int i) {
  char buf[8];
  Element *tmp;

  snprintf(buf,sizeof(buf)-1, "%d", i);
  tmp = new Element(std::string(), std::string(buf));
...
const Attribute *Element::operator [](const std::string &n) const
{
...
Attribute *Element::addAttribute(const std::string &n, const std::string &v)
{
  ...
  _proplist.push_back(tmp);

  return tmp;
}

Attribute *Element::addAttribute(const std::string &n, const int v)
{
  Attribute *tmp;
...
(Attribute * attributeP)
{
  ...
  _proplist.push_back(attributeP);
...
void writenode(xmlDocPtr doc, Element *n, xmlNodePtr p, int root)
{
  ...
  if (root) {
    ...
    node = xmlNewDocNode(doc, NULL, (xmlChar *) n->name().c_str(), 0);
    xmlDocSetRootElement(doc, node);
#if DDD
    std::cout << "new node: " <<  n->name().c_str();
#endif
  }
  else
    {
  ...
  if (n->isContent())
    {
      ...
      xmlNodeSetContentLen(node, (const xmlChar *) n->getContent().c_str(),
			   n->getContent().length());
#if DDD
      std::cout << " content: " << n->getContent().c_str();
#endif
    }
...
#if DDD
    std::cout << " prop: " <<  (*curprop)->name().c_str();
#endif
  }
