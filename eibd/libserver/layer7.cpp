...
Layer7_Broadcast::Layer7_Broadcast (Layer3 * l3, Logs * tr,  int inquemaxlen, int outquemaxlen, int peerquemaxlen) :
DroppableQueueInterface(tr)
{
  TRACEPRINTF (Loggers(), 5, this, "L7Broadcast Open");
  l4 = new T_Broadcast (l3, tr, 0, inquemaxlen, outquemaxlen, peerquemaxlen);
  if (!l4->init ())
    {
      delete l4;
      l4 = 0;
...
{
  TRACEPRINTF (Loggers(), 5, this, "L7Broadcast Close");
  if (l4)
     delete l4;
}
...
Layer7_Connection::Layer7_Connection (Layer3 * l3, eibaddr_t d,
				      Logs * tr,
				      ...
				      DroppableQueueInterface(tr)
{
  dest = d;
  l4 = 0;
  l4 = new T_Connection (l3, tr, d, inquemaxlen, outquemaxlen, peerquemaxlen);
  if (!l4->init ())
    {
      delete l4;
      l4 = 0;
...
Layer7_Individual::Layer7_Individual (Layer3 * l3, Logs * tr, eibaddr_t d ,  int inquemaxlen, int outquemaxlen, int peerquemaxlen) :
		DroppableQueueInterface(tr)
{
  dest = d;
  l4 = new T_Individual (l3, tr, d, false, inquemaxlen, outquemaxlen, peerquemaxlen);
  if (!l4->init ())
    {
      delete l4;
      l4 = 0;
