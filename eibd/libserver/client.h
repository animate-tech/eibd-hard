...
class ClientConnection:public Thread, public StateInterface
{
  /** client connection */
  int fd;
  /** Layer 3 interface */
  ...
  /** buffer length*/
  unsigned buflen;
  /** address connection is coming from */
...
public:
    ClientConnection (Server * s, Layer3 * l3,
		      Logs * tr,
  ...
  /// this is dumping basic client structure with some counters, rest to be done by subclass
  virtual Element * _xml(Element *parent) const;

  const char *str(void) { return "Client Connection"; }
private:
  const void LogBusMon(bool start) const;
};
